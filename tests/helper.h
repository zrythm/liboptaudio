...
#define oa_assert(x) \
  if (!(x)) \
    { \
      oa_printf("Assertion failed: %s", #x); \
      exit(1); \
    }
#else
#define oa_assert(x) \
...
#endif
#define oa_assert_cmpfloat_with_epsilon(n1,n2,epsilon) \
  oa_assert (fabsf (n1 - n2) < epsilon)
#define oa_assert_cmpfloat(n1,n2) \
  oa_assert (fabsf (n1 - n2) < FLT_EPSILON)
